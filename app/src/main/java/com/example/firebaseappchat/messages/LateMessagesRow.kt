...
class LateMessagesRow(val chatMessage: ChatMessage, val check: Boolean) :
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if (!check){
                        ...
                        if (chatPartnerUser?.Urlphoto?.isEmpty() == true){
                            ...
                            Picasso.get().load(IMGURL).into( viewHolder.itemView.imageview_lastest_messages)
                        }
                        else
                        {
                            Picasso.get().load(chatPartnerUser?.Urlphoto).into( viewHolder.itemView.imageview_lastest_messages)
                        }
                    }else{
